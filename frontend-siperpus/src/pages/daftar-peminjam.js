...
import { faArrowDownShortWide, faArrowUpShortWide, faPlus } from "@fortawesome/free-solid-svg-icons";
import ReactPaginate from "react-paginate";

export default class DaftarPeminjam extends Component {
  constructor(props) {
    super(props);
    ...
    this.filterPeminjam = this.filterPeminjam.bind(this);
    this.onChangeSortBy = this.onChangeSortBy.bind(this);
    this.handleSearch = this.handleSearch.bind(this);
    this.handleSortByPeminjam = this.handleSortByPeminjam.bind(this);
    this.handleSortByJumlahDipinjam = this.handleSortByJumlahDipinjam.bind(this);
    ...
    this.state = {
      ...
      loading: true,
      sortPeminjam: true,
      sortJumlahDipinjam: true,
      showSortPeminjam: false,
      showSortJumlahDipinjam: false,
  ...
  // Fungsi untuk menghitung jumlah buku yang dipinjam per pengguna
  filterPeminjam(){
    let lst1 = this.state.data
    let lst2 = this.state.hasil
    
    lst1.forEach(element =>{
    let obj = lst2.find(f=>f.idUser === element.user.id);
    if (obj) {
      obj.jumlahBuku += 1;
      }

    else {
      let pengguna = {idUser: element.user.id, namaUser: element.user.name, nipUser: element.user.nisn, emailUser: element.user.email , jumlahBuku:1};
      lst2.push(pengguna);
      }
    });
      ...
      this.setState({
        hasil: lst2,
        searchResult: lst2,
        countPage: Math.ceil(lst2.length / this.state.perPage),
        loading: false
      });
  ...
  handleSearch(event){
    ...
    // Search Filter
    let result = this.state.hasil.filter((peminjam) => {
      return peminjam.namaUser.toLowerCase().indexOf(search.toLowerCase()) !== -1})
  ...
  handleSortByPeminjam(){
    ...
    if (this.state.sortPeminjam) {
      result.sort((peminjamA, peminjamB) => (peminjamA.namaUser.toLowerCase() > peminjamB.namaUser.toLowerCase()) ? 1 : -1)
    }
  ...
  // Handle Sort By Jumlah Dipinjam
  handleSortByJumlahDipinjam(){
    let result = this.state.searchResult;

    // Sort By Jumlah Dipinjam
    if (this.state.sortJumlahDipinjam) {
      result.sort((peminjamA, peminjamB) => (peminjamA.jumlahBuku > peminjamB.jumlahBuku) ? 1 : -1)
    }
    ...
    this.setState({
      ...
      sortJumlahDipinjam: !this.state.sortJumlahDipinjam,
      sortPeminjam: true,

      showSortJumlahDipinjam: true,
      showSortPeminjam: false,
    });
  }
  ...
  onChangeSortBy(e){
    if(e.target.value === "buku"){
      this.setState({buku: false});
    }
  }
  ...
  render() {
    return (
      ...
      {this.state.buku ? <></> : <Navigate to={"/peminjaman/buku"}/>}
      ...
      <div className="m-10">
        ...
        <div className="flex flex-col mt-10">
          <div className="py-2 -my-2 overflow-x-auto sm:-mx-6 sm:px-6 lg:-mx-8 lg:px-8">
              <div className="inline-block min-w-full overflow-hidden align-middle border-b border-gray-200 shadow sm:rounded-lg">
                  <table className="min-w-full text-center">
                      ...
                      <thead>
                          <tr>
                              <th
                                  className="px-6 py-3 text-md font-bold leading-4  text-black ourFont border-b border-gray-500 bg-gray-50">
                                  No.</th>
                              <th
                              ...
                              <th
                                  className="px-6 py-3 text-md font-bold leading-4  text-black ourFont border-b border-gray-500 bg-gray-50 text-center">
                                  NIP
                              </th>
                              <th
                                  className="px-6 py-3 text-md font-bold leading-4  text-black ourFont border-b border-gray-500 bg-gray-50 text-center">
                              ...
                              <th
                                  className="px-6 py-3 text-md font-bold leading-4  text-center text-black ourFont border-b border-gray-500 bg-gray-50">
                                  <div className="flex justify-center">
                                    <p className="w-18">Jumlah Buku yang Dipinjam</p>
                                    {!this.state.showSortJumlahDipinjam ?
                                    <span onClick={this.handleSortByJumlahDipinjam} className="cursor-pointer ml-2 self-center text-gray-400">
                                      <FontAwesomeIcon icon={faArrowDownShortWide}/>
                                    </span>
                                    :
                                    <span onClick={this.handleSortByJumlahDipinjam} className="cursor-pointer ml-2">
                                      {this.state.sortJumlahDipinjam ? 
                                      <FontAwesomeIcon icon={faArrowUpShortWide}/>
                                      :
                                      <FontAwesomeIcon icon={faArrowDownShortWide}/>
                                      }
                      ...
                      <tbody className="bg-white">
                          {this.state.searchResult.slice(this.state.offsetPage, this.state.offsetPage + this.state.perPage).map((peminjam, no) => 
                            <tr key={peminjam.idUser}>
                                {/* No. */}
                                ...
                                {/* Detail Peminjam */}
                                <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                  <NavLink to={`/peminjam/${peminjam.idUser}`} className="btn-green text-center">Detail</NavLink>
                                </td>
                            </tr>
                          )}
                      </tbody>
            ...
            {/* Pengganti jika tidak ada peminjam buku*/}
        ...
        {this.state.successDenda? 
          <>
            <Popup type="success" pesan="Pengembalian Buku dan pembayaran denda berhasil dilakukan!" />
